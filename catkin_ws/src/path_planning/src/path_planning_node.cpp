...
int iFilter, indexFilter[1000], indexLingkaran[1000], iLingkaran;
int selisihIndex[1000];
...
void cariLingkaran(){
	for (int i = 0; i < iSelisih; i++){
		...
		if (indexSelisih[i] >= 180 && indexSelisih[i] <= 900){
	...
	for (int i = 0; i < iFilter; i++){
		...
		if (selisihIndex[i] >= 4 && selisihIndex[i] <= 100){
...
int cekAman(Coordinat titik, float range){
	int hasil = 0;
	float jarak = 0;
...
void evaluasi(float range){
	for (int i = 0; i < maxPopulasi; i++){
		...
		for (int j = 1; j < maxKromosom; j++){
			...
			hasilAman = cekAman(tempPoint[j], range);
...
int main(int argc, char **argv) {
	...
	printf("\nTitik tengah tujuan : (%.2f , %.2f)\n", Goal.x, Goal.y);
	printf("Titik tengah halangan : (%.2f , %.2f)\n\n", Obs.x, Obs.y);
	for (int k = 0; k < maxKromosom; k++){
		printf("Distance = %.2f\n", maxDistance[k]);
	}
	...
	//Konfigurasi Tujuan
		...
		goal.color.g = 1.0f;
		goal.color.a = 1.0;
		goal.pose.position.x = Goal.y/100;
		goal.pose.position.y = -1*Goal.x/100;
		goal.pose.position.z = 0.0;

	//Konfigurasi Halangan
		obs.scale.x = 0.12;
		obs.scale.y = 0.12;
		obs.scale.z = 0.2;
		obs.color.r = 1.0f;
		obs.color.a = 1.0;
		obs.pose.position.x = Obs.y/100;
		obs.pose.position.y = -1*Obs.x/100;
		obs.pose.position.z = 0.0;

	//Konfigurasi Jalur Robot
		path.scale.x = 0.01;
		path.color.b = 1.0f;
		path.color.a = 1.0;
