...
void jarakPintu(int iIndex, int index[], int iCorner, int indexCorner[], Coordinat corner[], int minSelisih){
	...
	if (flagCornerPintu == 1){
		...
		for (int i = 0; i < ititikPintu; i++){
			for (int j = 0; j < iSeleksi; j++){
				selisihDoor[i] = abs(titikPintu[i] - indexCorner[j]);
				if (selisihDoor[i] > 0 && selisihDoor[i] < 300){
					indexCornerPintu = indexCorner[j];
					iCornerPintu = j;	
					//flagCornerPintu = 1;
				}
		...
		for (int i = 0; i < ititikPintu; i++){
			if (jarakDoorCorner[i] > 43 && jarakDoorCorner[i] < 55){
				...
				printf("Door Detected \n");
				 ...
				 for (int i = 0; i < iSeleksi; i++){
					 ...
					 else if(selisihCornerPintu[i] > 500){
						//antarCorner = distanceFrom(titikCorner[i], titikCorner[iCornerPintu]);
					 	flagCorner[i] = 1;
					 	resultRoom = 0;
					 }
			...
			for (int i = 0; i < iSeleksi; i++){	
				if (flagCorner[i] == 1){
					...
					else if (jarakCorner[i] > 106 && jarakCorner[i] < 119){
						resultRoom = 3;
						printf("Corner Diagonal 1\n");
					}
					...
					else if (jarakCorner[i] > 49 && jarakCorner[i] < 55){
						resultRoom = 4;
						refRoom = 2;
						printf("Corner Parallel 1\n");
					}
					else if (jarakCorner[i] > 84 && jarakCorner[i] < 91){
						resultRoom = 3;
						refRoom = 4;
						printf("Corner Parallel 1\n");
					}
...
int main(int argc, char **argv) {
	...
	if(iSeleksi>1){
		...
		printf("Putar sensor 90 derajat\n");
	}
